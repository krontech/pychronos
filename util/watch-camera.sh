...
#Stop the autostarted one from running, if available.
systemctl stop chronos-control         2> /dev/null || true
systemctl stop chronos-control-api-dev 2> /dev/null || true
