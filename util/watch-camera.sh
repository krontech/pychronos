...
bash <<< "#sh doesn't do the equality test for 143, must use bash
	while true; do
		sleep 2 &
		python3 cam-newapi < `readlink -f /dev/stdin` 2> `readlink -f /dev/stderr`
		PY_EXIT=\$?
...
def callback(evt):
	...
	killingProcess = subprocess.Popen(f"sleep {timeout} && pkill --signal SIGKILL --parent {sys.argv[1]} --full cam-newapi", shell=True)
